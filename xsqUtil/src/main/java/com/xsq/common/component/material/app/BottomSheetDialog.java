package com.xsq.common.component.material.app;
...
import com.xsq.common.component.material.drawable.BlankDrawable;
import com.xsq.common.R;

import static com.xsq.common.R.*;
...
public class BottomSheetDialog extends android.app.Dialog{
    ...
    public BottomSheetDialog(Context context) {
        this(context, style.Material_App_BottomSheetDialog);
    }
    ...
    public BottomSheetDialog applyStyle(int styleId) {
        ...
        TypedArray a = context.obtainStyledAttributes(styleId, styleable.BottomSheetDialog);

        for(int i = 0, count = a.getIndexCount(); i < count; i++){
            int attr = a.getIndex(i);
            if(attr == styleable.BottomSheetDialog_android_layout_height)
                heightParam(a.getLayoutDimension(attr, ViewGroup.LayoutParams.WRAP_CONTENT));
            else if(attr == styleable.BottomSheetDialog_bsd_cancelable)
                cancelable(a.getBoolean(attr, true));
            else if(attr == styleable.BottomSheetDialog_bsd_canceledOnTouchOutside)
                canceledOnTouchOutside(a.getBoolean(attr, true));
            else if(attr == styleable.BottomSheetDialog_bsd_dimAmount)
                dimAmount(a.getFloat(attr, 0f));
            else if(attr == styleable.BottomSheetDialog_bsd_inDuration)
                inDuration(a.getInteger(attr, 0));
            else if(attr == styleable.BottomSheetDialog_bsd_inInterpolator) {
                int resId = a.getResourceId(attr, 0);
                if(resId != 0)
                    inInterpolator(AnimationUtils.loadInterpolator(context, resId));
            }
            else if(attr == styleable.BottomSheetDialog_bsd_outDuration)
                outDuration(a.getInteger(attr, 0));
            else if(attr == styleable.BottomSheetDialog_bsd_outInterpolator) {
                int resId = a.getResourceId(attr, 0);
                if(resId != 0)
